...
def render(key_prefix: str = "", *, use_sidebar: bool = True) -> None:
    ...
    page_header(
        title="Main Demo — AI Investments vs US New Business Apps",
        subtitle="Compare AI investment growth with US new business application growth.",
        badge="Innovation",
    )
    with st.expander("Presenter talking points", expanded=False):
        st.markdown(
            """
            - This panel compares AI investment growth with US new business application growth.
            - The purpose is to frame the broader market context for AI, cloud, and innovation governance.
            - AI investment growth can be volatile, so organizations should avoid chasing hype without readiness.
            - Management implication: Crimson 5 should connect AI investment to governance, data quality, adoption, and measurable business outcomes.
            """
        )
    st.markdown(
        "This external trend demo compares AI investment growth with US business application growth to frame why cloud, AI, and innovation governance matter for Crimson 5 Energy."
    )
    with st.expander("Assumptions used", expanded=False):
        st.markdown(
            """
            - Defaults use historical AI investment and US new business application inputs.
            - Sliders let you adjust yearly values and stress-test scenarios.
            - Output compares growth rates, trends, and the gap between the two series.
            - Forecasts use a simple linear trend and are illustrative, not predictive guidance.
            """
        )
    ...
    if use_sidebar:
        ...
        with st.expander("Adjust AI Investments ($ Billions), 2015–2023", expanded=not presentation_mode):
            _collect_ai(st)
        with st.expander("Adjust US New Business Apps (Millions), 2015–2023", expanded=not presentation_mode):
            _collect_us(st)
    ...
    chart_caption("Notice when AI growth surges outpace business formation and create execution pressure.")
    ...
    chart_caption("Trend direction matters more than any single year when assessing investment momentum.")
    ...
    fig_gap.update_layout(
        title="Gap between AI and business app growth",
        xaxis_title="Year",
        yaxis_title="Gap (percentage points)",
    )
    apply_crimson_layout(fig_gap, height=350)
    st.plotly_chart(fig_gap, use_container_width=True)
    chart_caption("Positive gaps show periods where AI investment growth outpaced business application growth.")

    st.subheader("Data Tables")
    hist_df = pretty_columns(df)
    pred_df = pretty_columns(df_pred)
    with st.expander("View detailed table", expanded=not presentation_mode):
        ...
        with c1:
            ...
            dataframe_download(hist_df, "main_demo_historical_growth.csv", label="Download historical CSV")
        with c2:
            ...
            dataframe_download(pred_df, "main_demo_forecast_growth.csv", label="Download forecast CSV")

    chart_caption("Sliders update charts and linear-regression forecasts automatically.")
    st.markdown("##### Final interpretation")
    st.markdown(
        """
        - AI investment growth is volatile but strategically important.
        - Business application growth indicates entrepreneurial demand.
        - The gap analysis helps frame timing risk and innovation opportunity.
        - Crimson 5 should avoid chasing AI spend alone; it should connect AI investment to data quality, product adoption, and governance.
        """
    )
    executive_insight(
        "The gap between AI investment growth and business formation suggests opportunity, but also volatility and timing risk."
    )
