...
def render(key_prefix: str = "p7_") -> None:
    kp = key_prefix
    page_header(
        title="AI ROI Explorer",
        subtitle="Explore how AI investment and data quality influence return potential.",
        badge="Innovation",
    )
    with st.expander("Presenter talking points", expanded=False):
        st.markdown(
            """
            - This panel connects AI investment to data quality and ROI potential.
            - AI value depends not only on budget, but on whether the organization has usable data and operational readiness.
            - The under-invest, balanced, and over-invest zones help frame investment discipline.
            - Management implication: AI funding should be tied to data maturity, adoption readiness, and measurable use cases.
            """
        )
    with st.expander("Assumptions used", expanded=False):
        st.markdown(
            """
            - Defaults assume mid-range AI spend and data quality.
            - Sliders control investment amount and data quality score.
            - Output shows modeled ROI and where the current choice sits on the curve.
            - This is a directional curve model and does not represent project-level P&L.
            """
        )
    ...
    executive_insight(
        "AI ROI depends not only on model investment, but also on data quality and readiness to operationalize insights."
    )
    ...
    fig.update_layout(
        title="ROI vs AI investment level",
        xaxis_title="AI investment ($)",
        yaxis_title="Modeled ROI (%)",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
    apply_crimson_layout(fig)
    st.plotly_chart(fig, use_container_width=True)
    chart_caption("Returns flatten at higher spend unless data quality is strong.")
    st.markdown(
        """
